 ...
 * @date   2010
 * @brief  A class for representing images by <a href="http://labe.felk.cvut.cz/~tkrajnik/">Tom Krajnik</a>.
 *
*/



//! Simple image class by <a href="http://labe.felk.cvut.cz/~tkrajnik/">Tom Krajnik</a>.
class CRawImage
{
	public:
		...
		//! saves the image to a file with bmp format, calling this consecutivelly causes to save files with names 0000.bmp, 0001.bmp, 0002.bmp etc. 
		void saveBmp();

		//! checks if there are images in the directory and sets the saving number in order to avoid overwriting them
		int  getSaveNumber();

		//! loads a file in BMP format usefull only for bitmaps created with saveBmp(...) functions 
