        ...
        for time_index = 1 : MAX_TIME
            ...
            if (mod(time_index, RUN_PERIOD) == 1)% always run at time 1
                ...
                %while ((uncoveredNum_fuzzy >= floor(UNCOVERED_FRACTION_CRITERION * TargetCount*MAX_PREDICTION))  && (cluster_count_fuzzy < MAX_CLUSTER))
                    Result = FuzzyClusteringMATLABFunc(data, AOV_degree, R_min, R_max, Dim1_min, Dim1_max, Dim2_min, Dim2_max, MAX_ITERATION, UNCOVERED_FRACTION_CRITERION, CamCountFuzzy_main, TargetCount*MAX_PREDICTION, angle_coeff, blend, multi_class_probability)
                    for temp_c = 1 : size(Result,1)
                        if Result(temp_c, 1) == -2
                            membership_row = temp_c + 1;
                            break
                ...
                Result;
                CamCountFuzzy = CamCountFuzzy_main
