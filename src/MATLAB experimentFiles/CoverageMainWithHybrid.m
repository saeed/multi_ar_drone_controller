...
for s = 1 : ScenNum
    ...
    while ((uncoveredNum_algo1 > floor(UNCOVERED_FRACTION_CRITERION * TargetCount))  & (cluster_count_algo1 < MAX_CLUSTER))
        Result = ClusteredCoverageHybridFunc(data, AOV_degree, R_min, R_max, Dim1_min, Dim2_min, Dim1_max, Dim2_max, MAX_ITERATION, UNCOVERED_FRACTION_CRITERION ,cluster_count_algo1, TargetCount,angle_coeff, polar);
        %Result = TreeClusteredCoverageHybridFunc(data, AOV_degree, R_min, R_max, Dim1_min, Dim2_min, Dim1_max, Dim2_max, MAX_ITERATION, UNCOVERED_FRACTION_CRITERION ,cluster_count_algo1, TargetCount,angle_coeff);
...
uncovered_avg_algo1_kmeans = uncovered_avg_algo1 / ScenNum
cluster_count_avg_algo1_kmeans = cluster_count_avg_algo1 / ScenNum
...
title('Algorithm Kcam-Hybrid-Kmeans clusters ');
...
for s = 1 : ScenNum
    ...
    tstart = tic;
    while ((uncoveredNum_algo1 > floor(UNCOVERED_FRACTION_CRITERION * TargetCount))  & (cluster_count_algo1 < MAX_CLUSTER))
        %Result = ClusteredCoverageHybridFunc(data, AOV_degree, R_min, R_max, Dim1_min, Dim2_min, Dim1_max, Dim2_max, MAX_ITERATION, UNCOVERED_FRACTION_CRITERION ,cluster_count_algo1, TargetCount,angle_coeff);
        Result = TreeClusteredCoverageHybridFunc(data, AOV_degree, R_min, R_max, Dim1_min, Dim2_min, Dim1_max, Dim2_max, MAX_ITERATION, UNCOVERED_FRACTION_CRITERION ,cluster_count_algo1, TargetCount,angle_coeff);
        %uncoveredNum_algo1 = Result( 1 , 1 );
        uncoveredNum_algo1 = Result( 1 , 5 ); 
        uncovered_algo1_vector(cluster_count_algo1) = uncoveredNum_algo1;
...
uncovered_avg_algo1_tree = uncovered_avg_algo1 / ScenNum
cluster_count_avg_algo1_tree = cluster_count_avg_algo1 / ScenNum
%complexity_avg_algo1_tree = complexity_avg_algo1 / ScenNum
figure;
x = ClusterNum : cluster_count_algo1;
true_x = actual_cam_num(x)
plot(x,uncovered_algo1_vector(ClusterNum:cluster_count_algo1));
xlabel('#cameras');
ylabel('#uncovered targets');
title('Algorithm Kcam-Hybrid Tree ');
