function coverage_status = CoverageTestMultiClusterFunc(targets, aov, Rmin, Rmax,Dim1_min,Dim2_min, Dim1_max, Dim2_max, clusterIndex )
    ...
    for i = 1 : size(targets,2)
        if ( (targets(i).cluster_index1 == clusterIndex) || (targets(i).cluster_index2 == clusterIndex) )
    ...
    coverage_status = [CoverageStatus_dummy, -1, CAM_dummy, best_line_slope_dummy, complexity ];
    ...
    if ( ( TargetNum > 0 )  &  ( TargetNum < 2 ) )
        ...
        coverage_status = [CoverageStatus , -1, CAM, best_line_slope_dummy, complexity];
    end
...
%     end
    ...
    if ( TargetNum == 2 )
        ...
        if ( dist > R_max )
            ...
            coverage_status = [ 1, 0, -1, CAM , best_line_slope, complexity];
            return
        end
        if (X(1) < X(2))
            indicator = 1;
        ...
        coverage_status = [ 1, 1, -1, x_CAM, y_CAM, z_CAM, best_line_slope, complexity];
        return
    end
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
