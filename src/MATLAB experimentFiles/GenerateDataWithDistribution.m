...
function my_data = GenerateDataWithDistribution(dim1, dim2, TargetNum, targetClusters, seed)
...
% Z = zeros(1,TargetNum);
 ...
 if targetClusters == 1
    ...
    Y = ((dim/4).*randn(TargetNum,1) + dim/2)';
 elseif targetClusters == 2
     X1 = (dim/4).*randn(TargetNum/2,1) + dim/2;
     Y1 = (dim/8).*randn(TargetNum/2,1) + dim/4;
     X2 = (dim/4).*randn(TargetNum/2,1) + dim/2;
     Y2 = ((dim)/8).*randn(TargetNum/2,1) + (3*dim)/4;
     X = ([X1;X2])';
     Y = ([Y1;Y2])';
 elseif targetClusters == 4
     X1 = (dim/8).*randn(floor(TargetNum/4),1) + dim/4;
     Y1 = (dim/8).*randn(floor(TargetNum/4),1) + dim/4;
     X2 = (dim/8).*randn(floor(TargetNum/4),1) + (3*dim)/4;
     Y2 = (dim/8).*randn(floor(TargetNum/4),1) + dim/4;
     X3 = (dim/8).*randn(floor(TargetNum/4),1) + dim/4;
     Y3 = (dim/8).*randn(floor(TargetNum/4),1) + (3*dim)/4;
     X4 = (dim/8).*randn(floor(TargetNum/4)+ mod(TargetNum,targetClusters), 1) + (3*dim)/4;
     Y4 = (dim/8).*randn(floor(TargetNum/4) + mod(TargetNum,targetClusters) , 1) + (3*dim)/4;
     X = ([X1;X2;X3;X4])';
     Y = ([Y1;Y2;Y3;Y4])';
