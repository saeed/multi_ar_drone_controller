    ...
    for s = 1 :ScenNum
        ...
        for time_index = 1 : MAX_TIME
            ...
            %else
                ...
                CurrentCams;
                coverage = LightCoverageTest(real_data, membership, CurrentCams, AOV_degree, R_min, R_max, Dim1_min, Dim2_min, Dim1_max, Dim2_max, membership_row -2, TargetCount);
...
%for TargetCount = 10: 10: 100
    ...
    for s = 1 :ScenNum
        ...
        for time_index = 1 : MAX_TIME
            ...
            if (mod(time_index, RUN_PERIOD) == 1)% always run at time 1
                ...
                for future_time = 1 : MAX_PREDICTION
                end
                for k = 1 : TargetCount
                    mainDataSet(k,:) = [locations(k,time_index); locations(k+TargetCount,time_index); locations(k+2*TargetCount,time_index)];
                    if (time_index + MAX_PREDICTION > MAX_TIME)
            ...
            %else
                ...
                CurrentCams;
                coverage = LightFuzzyCoverageTest(real_data, membership, CurrentCams, AOV_degree, R_min, R_max, Dim1_min, Dim2_min, Dim1_max, Dim2_max, membership_row -2, TargetCount)
                covered = sum(coverage)
  ...
  figure;
  plot(sum(uncoveredNum_fuzzy_avg,2)/MAX_TIME);
  xlabel('speed');
