function Result = ClusteredCoverageFunc(data,AOV_degree, R_min, R_max, Dim1_min, Dim2_min, Dim1_max, Dim2_max, max_iteration, uncovered_fraction_criterion, ClusterNum, TargetCount,margin)
...
%         end
%         cluster_cam_slopes( i ) = status( i , j + 4 );
%         cluster_cam_dirs(: , i) = status(i, j + 4 : j + 5);
%         complexity = complexity + status( i , j + 6 );
%     end
for i = 1 : ClusterNum
        if (sum(status(i,:),2) == (-1)*size(status,2))
            coverage_status(i, :) = status(i, :);
