...
UNCOVERED_FRACTION_CRITERION = 0.2;%0.2; 
MAX_ITERATION = 50;
...
%AOV_vec = [30, 35, 40, 45, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 160, 170, 180];
%AOV_vec = [90];
if 0
t = 0; %target counter
...
figure;
plot(AOV_vec, cluster_count_avg_algo1)
xlabel('AOV');
ylabel('avg. #cameras');
title('Algorithm 1 ');

figure;
plot(AOV_vec, uncovered_avg_algo1);
xlabel('#AOV');
ylabel('#uncovered');
title('Algorithm 1 ');

figure;
plot(AOV_vec, exec_time_avg);
xlabel('AOV');
...
%for TargetCount = 10: 10: 100
  ...
  figure;
  plot(AOV_vec,camNum_avg);
  xlabel('AOV');
  ylabel('#cameras');
  title('Algorithm 2 ');
  figure;
  plot(AOV_vec, uncoveredNum_algo2_avg);
  xlabel('#AOV');
  ylabel('#uncovered');
  title('Algorithm 2 ');
  ...
  plot(AOV_vec, exec_time_algo2_avg);
  xlabel('AOV');
  ylabel('execution time');
  title('Algorithm 2 ');
...
% k = 1;
% while  uncoveredNum_algo3 < 
%     uncoveredNum_algo3 = FuzzyClusteringMATLABFunc(seed,AOV_degree, R_min, R_max, Dim1_max, Dim2_max, MAX_ITERATION, UNCOVERED_FRACTION_CRITERION, cluster_count_algo3, TargetCount, angel_coeff, blend, multi_class_probability);
...
%for TargetCount = 10: 10: 100
  ...
  figure;
  plot(AOV_vec,camNum_avg_fuzzy);
  xlabel('AOV');
  ylabel('#cameras');
  title('Algorithm Fuzzy ');
  figure;
  plot(AOV_vec, uncoveredNum_fuzzy_avg);
  xlabel('#AOV');
  ylabel('#uncovered');
  title('Algorithm Fuzzy ');
  ...
  figure;
  %plot(UncoveredVsCamNum2(1:CamCountAlgo2));
  plot(AOV_vec, exec_time_fuzzy_avg);
  xlabel('AOV');
  ylabel('execution time');
  title('Algorithm Fuzzy ');
