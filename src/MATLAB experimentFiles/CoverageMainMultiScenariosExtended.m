...
plot(x,uncovered_algo1_vector(s,ClusterNum:cluster_count_algo1));
xlabel('#cameras');
...
cluster_count_sum_hybrid_kmeans = 0;
exec_time_sum_hybrid_kmeans = 0;
uncovered_sum_hybrid_kmeans = 0;

uncovered_vector_hybrid_kmeans = zeros(ScenNum,MAX_CLUSTER)-1;
uncovered_sum_vector_hybrid_kmeans = zeros(ScenNum,MAX_CLUSTER);
for i = 1 : ScenNum
    uncovered_vector_hybrid_kmeans(i,1:ClusterNum-1) = TargetCount;
    uncovered_sum_vector_hybrid_kmeans(i,1:ClusterNum-1) = 0;
end
...
for s = 1 : ScenNum
    seed = s;
    data = GenerateData(Dim1_max, Dim2_max, TargetCount, seed);
    uncoveredNum_hybrid_kmeans = TargetCount;
    cluster_count_hybrid_kmeans = 1;% ClusterNum;
    
    complexity_algo1 = 0;
    tstart = tic;
    while ((uncoveredNum_hybrid_kmeans > floor(UNCOVERED_FRACTION_CRITERION * TargetCount)) && (cluster_count_hybrid_kmeans < MAX_CLUSTER))
        Result = ClusteredCoverageHybridFunc(data, AOV_degree, R_min, R_max, Dim1_min, Dim2_min, Dim1_max, Dim2_max, MAX_ITERATION, UNCOVERED_FRACTION_CRITERION ,cluster_count_hybrid_kmeans, TargetCount,angle_coeff, use_polar);
         %uncoveredNum_algo1 = Result( 1 , 1 );
        uncoveredNum_hybrid_kmeans = Result( 1 , 5 ) 
        uncovered_vector_hybrid_kmeans(s,cluster_count_hybrid_kmeans) = uncoveredNum_hybrid_kmeans;
        uncovered_vector_hybrid_kmeans
        %complexity_algo1 = complexity_algo1 + Result( 1 , 2 );
        
        actual_cam_num(cluster_count_hybrid_kmeans) = size(Result,1);
        cluster_count_hybrid_kmeans = cluster_count_hybrid_kmeans + 1;
    end
    cluster_count_hybrid_kmeans = cluster_count_hybrid_kmeans - 1;
    elapse_hybrid_kmeans = toc(tstart);
    uncovered_sum_vector_hybrid_kmeans(s,cluster_count_hybrid_kmeans)= uncovered_sum_vector_hybrid_kmeans(s,cluster_count_hybrid_kmeans) + uncovered_vector_hybrid_kmeans(s,cluster_count_hybrid_kmeans);
    uncovered_sum_hybrid_kmeans = uncovered_sum_hybrid_kmeans + uncovered_vector_hybrid_kmeans(s,cluster_count_hybrid_kmeans);
    cluster_count_sum_hybrid_kmeans = cluster_count_sum_hybrid_kmeans + cluster_count_hybrid_kmeans;
    exec_time_sum_hybrid_kmeans = exec_time_sum_hybrid_kmeans + elapse_hybrid_kmeans;
end

uncovered_avg_hybrid_kmeans = uncovered_sum_hybrid_kmeans / ScenNum
cluster_count_avg_hybrid_kmeans = cluster_count_sum_hybrid_kmeans / ScenNum
exec_time_avg_hybrid_kmeans = exec_time_sum_hybrid_kmeans / ScenNum
figure;
x = ClusterNum : cluster_count_hybrid_kmeans;
true_x = actual_cam_num(x)
plot(x,uncovered_vector_hybrid_kmeans(s,ClusterNum:cluster_count_hybrid_kmeans));
xlabel('#cameras');
ylabel('#uncovered targets');
title('Algorithm Hybrid - kmeans ');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cluster_count_sum_hybrid_tree = 0;
exec_time_sum_hybrid_tree = 0;
uncovered_sum_hybrid_tree = 0;

uncovered_vector_hybrid_tree = zeros(ScenNum,MAX_CLUSTER);
uncovered_sum_vector_hybrid_tree = zeros(ScenNum,MAX_CLUSTER);
for i = 1 : ScenNum
    uncovered_vector_hybrid_tree(i,1:ClusterNum-1) = TargetCount;
    uncovered_sum_vector_hybrid_tree(i,1:ClusterNum-1) = 0;
end
...
for s = 1 : ScenNum
    seed = s;
    data = GenerateData(Dim1_max, Dim2_max, TargetCount, seed);
    uncoveredNum_hybrid_tree = TargetCount;
    cluster_count_hybrid_tree = 1;% ClusterNum;
    
    complexity_algo1 = 0;
    tstart = tic;
    while ((uncoveredNum_hybrid_tree > floor(UNCOVERED_FRACTION_CRITERION * TargetCount)) && (cluster_count_hybrid_tree < MAX_CLUSTER))
        Result = TreeClusteredCoverageHybridFunc(data, AOV_degree, R_min, R_max, Dim1_min, Dim2_min, Dim1_max, Dim2_max, MAX_ITERATION, UNCOVERED_FRACTION_CRITERION ,cluster_count_hybrid_tree, TargetCount,angle_coeff);
         %uncoveredNum_algo1 = Result( 1 , 1 );
        uncoveredNum_hybrid_tree = Result( 1 , 5 ); 
        uncovered_vector_hybrid_tree(s,cluster_count_hybrid_tree) = uncoveredNum_hybrid_tree;
        %complexity_algo1 = complexity_algo1 + Result( 1 , 2 );
        
        actual_cam_num(cluster_count_hybrid_tree) = size(Result,1);
        cluster_count_hybrid_tree = cluster_count_hybrid_tree + 1;
    end
    cluster_count_hybrid_tree = cluster_count_hybrid_tree - 1;
    elapse_hybrid_tree = toc(tstart);
    uncovered_sum_vector_hybrid_tree(s,cluster_count_hybrid_tree)= uncovered_sum_vector_hybrid_tree(s,cluster_count_hybrid_tree) + uncovered_vector_hybrid_tree(s,cluster_count_hybrid_tree);
    uncovered_sum_hybrid_tree = uncovered_sum_hybrid_tree + uncovered_vector_hybrid_tree(s,cluster_count_hybrid_tree);
    cluster_count_sum_hybrid_tree = cluster_count_sum_hybrid_tree + cluster_count_hybrid_tree;
    exec_time_sum_hybrid_tree = exec_time_sum_hybrid_tree + elapse_hybrid_tree;
end

uncovered_avg_hybrid_tree = uncovered_sum_hybrid_tree / ScenNum
cluster_count_avg_hybrid_tree = cluster_count_sum_hybrid_tree / ScenNum
exec_time_avg_hybrid_tree = exec_time_sum_hybrid_tree / ScenNum
figure;
x = ClusterNum : cluster_count_hybrid_tree;
true_x = actual_cam_num(x)
plot(x,uncovered_vector_hybrid_tree(s,ClusterNum:cluster_count_hybrid_tree));
xlabel('#cameras');
ylabel('#uncovered targets');
title('Algorithm Hubrid - Tree ');
...
for s = 1 :ScenNum
    ...
    while ((uncoveredNum_algo2 > UNCOVERED_FRACTION_CRITERION * TargetCount)  && (cluster_count_algo2 < MAX_CLUSTER))
        ResultAlgo2 = ConvertedFeaturesAlgorithmWorkableFunc(data, AOV_degree, R_min, R_max, Dim1_min, Dim1_max, Dim2_min, Dim2_max, MAX_ITERATION, UNCOVERED_FRACTION_CRITERION, cluster_count_algo2, TargetCount,angle_coeff);
